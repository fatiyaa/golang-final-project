...
const MyReservation = () => {
    ...
    useEffect(() => {
        ...
        const fetchbookingData = async (token: string) => {
            try {
                const response = await axios.get('http://127.0.0.1:8888/api/order', {
                headers: { 
                    ...
                    {bookingData.map((booking) => (
                        <div key={booking.id} className='max-w-3xl bg-white rounded-t-lg rounded-b-md flex flex-col gap-1 shadow-lg max-w-[20rem] h-[27rem]'>
                            ...
                            <div className='flex flex-col px-3 my-2 gap-1 h-full'>
                                ...
                                <div className='text-2xl font-semibold uppercase leading-5 tracking-[0.0375rem] text-gray-900 '>Hotel Name</div>
                                <div className='font-bold -mb-2 text-gray-700 text-base'>{booking.room.name}</div>
                                ...
                                <div className='flex flex-col h-full justify-between'>
                                    <div>
                                        ...
                                        <div className='-ml-1'>Total : Rp {new Intl.NumberFormat('id-ID').format(booking.room.base_price)}</div>
                                    ...
                                    <div className='flex flex-row  mb-2'>
                                        {booking.status === 'BOOKED' ? (
                                            <button className='bg-blue-600 text-white w-full py-2 px-4 rounded-xl shadow-xl cursor-pointer hover:bg-blue-700'>
                                            ...
                                            <div className='text-yellow-500 justify-center flex flex-col w-full'>
                                              ...
                                              <button className='mt-1 bg-red-600 text-white w-full py-2 px-4 rounded-xl shadow-xl cursor-pointer hover:bg-red-700 font-bold tracking-[0.2rem] uppercase'>
                                                Cancel Booking
                                              </button>
                                            </div>
                                          ) : booking.status === 'CHECKED_IN' ? (
                                            <button className='bg-red-600 text-white w-full py-2 px-4 rounded-xl shadow-xl cursor-pointer hover:bg-red-700'>
                                              <p className='font-bold tracking-[0.2rem]'>CHECKOUT NOW</p>
                                            </button>
                                          ) : booking.status === 'CHECKED_OUT' ? (
                                            <div className='text-blue-800 w-full'>
                                              <button className='mt-2 w-full bg-blue-600 text-white py-2 px-4 rounded-lg shadow-xl hover:bg-blue-700 font-bold tracking-[0.04rem]'>
